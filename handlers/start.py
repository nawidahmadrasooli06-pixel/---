...
def start_reply_keyboard(lang):
    return ReplyKeyboardMarkup([[KeyboardButton(t(lang, "start_button"))]], resize_keyboard=True, is_persistent=True, input_field_placeholder="پیام خود را بنویسید…")
...
async def _show_or_edit_menu(message, context, lang, user_id, edit=False):
    ...
    markup = main_menu_keyboard(lang, user_id == ADMIN_ID)
    if edit:
        try:
            await message.edit_text(menu_text(lang), reply_markup=markup)
            context.user_data["menu_message_id"] = message.message_id
            return
        except Exception:
            pass
    sent = await message.reply_text(menu_text(lang), reply_markup=markup)
    context.user_data["menu_message_id"] = sent.message_id
    try:
        await message.get_bot().set_my_commands([])
    except Exception:
        pass
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # A /start or reply-keyboard Start should immediately open the menu.
...
async def language_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await query.message.edit_text(menu_text(lang), reply_markup=main_menu_keyboard(lang, query.from_user.id == __import__("config").ADMIN_ID))
    except Exception:
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    data = query.data
    lang = context.user_data.get("lang") or get_user_language(query.from_user.id)
    ...
    if data == "menu_active":
        ...
        if not items:
            await query.message.edit_text(t(lang, "active_empty"), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(t(lang, "btn_back"), callback_data="menu_back")]]))
            return
        ...
        await query.message.edit_text(t(lang, "active_title"), reply_markup=InlineKeyboardMarkup(rows))
        return
    if data == "menu_stats":
        items = user_active_participations(query.from_user.id)
        if not items:
            await query.message.edit_text(t(lang, "stats_none"), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(t(lang, "btn_back"), callback_data="menu_back")]]))
            return
        if len(items) == 1:
            from handlers.participant import render_user_stats
            await render_user_stats(query, context, items[0][0], items[0][1])
            return
        rows = [[InlineKeyboardButton(f"📢 {c.get('title','چالش')}", callback_data=f"stats_{c['_id']}")] for c, _ in items]
        rows.append([InlineKeyboardButton(t(lang, "btn_back"), callback_data="menu_back")])
        await query.message.edit_text("📊 چالش موردنظر را انتخاب کن:", reply_markup=InlineKeyboardMarkup(rows))
        return
    if data == "menu_results":
        await query.message.edit_text(t(lang, "results"), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(t(lang, "btn_back"), callback_data="menu_back")]]))
        return
    if data == "menu_settings":
        await query.message.edit_text(t(lang, "choose_language"), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(t(lang, "btn_fa"), callback_data="lang_fa"), InlineKeyboardButton(t(lang, "btn_en"), callback_data="lang_en")], [InlineKeyboardButton(t(lang, "btn_back"), callback_data="menu_back")]]))
        return
    if data == "menu_about":
        await query.message.edit_text(t(lang, "about"), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(t(lang, "btn_back"), callback_data="menu_back")]]))
        return
    if data == "menu_creator":
        from handlers.about import creator_callback
        await creator_callback(update, context)
        return
    if data == "admin_panel":
    ...
    if data.startswith("challenge_view_"):
        ...
        if not c or not c.get("active"):
            await query.message.edit_text(t(lang, "challenge_closed"), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(t(lang, "btn_back"), callback_data="menu_active")]]))
            return
        ...
        await query.message.edit_text(text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(t(lang, "join"), callback_data=f"challenge_join_{challenge_id}")], [InlineKeyboardButton(t(lang, "btn_back"), callback_data="menu_active")]]))
        return
    ...
    if data.startswith("stats_"):
        ...
        if not found:
            ...
            return
        from handlers.participant import render_user_stats
        await render_user_stats(query, context, found[0], found[1])
        return
    ...
    if data.startswith("report_open_"):
        ...
        await open_report_flow(query, context, data.split("_", 2)[2])
        return
    if data.startswith("report_"):
        from handlers.participant import choose_report_reason
        await choose_report_reason(query, context, data)
        return
    if data.startswith("admin_"):
        from handlers.admin import dispatch_admin_callback
        await dispatch_admin_callback(update, context)
        return


async def handle_text_start_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
